...
class BaseHolder : protected BASE
{
  ...
  public:
    ...
    FORCEINLINE BaseHolder(BaseHolder&& other)
        :BaseHolder()
    {
    ...
    FORCEINLINE BaseHolder& operator=(BaseHolder&& other)
    {
        STATIC_CONTRACT_WRAPPER;
...
class StateHolder
{
  ...
  public:
    ...
    StateHolder(StateHolder&&) = default;
    StateHolder& operator=(StateHolder&&) = default;
...
class ConditionalStateHolder
{
  ...
  public:
    ...
    ConditionalStateHolder(const ConditionalStateHolder&) = delete;
    ConditionalStateHolder& operator=(const ConditionalStateHolder&) = delete;
    ConditionalStateHolder(ConditionalStateHolder&&) = default;
    ConditionalStateHolder& operator=(ConditionalStateHolder&&) = default;
...
private:
    INDEBUG_AND_WINDOWS_FOR_HOLDERS(TYPE* m_pvalue;)
};  // Wrapper<>
...
template<typename TYPE, void(*ReleaseF)(TYPE*)>
using PreallocatedWrapper = Wrapper<TYPE*, DoNothing, ReleaseF, NULL>;
...
template <typename TYPE>
using StubHolder = PreallocatedWrapper<TYPE, StubRelease<TYPE>>;
...
template <typename TYPE>
using CoTaskMemHolder = PreallocatedWrapper<TYPE, DeleteCoTaskMem<TYPE>>;
...
template <typename TYPE>
using NewHolder = PreallocatedWrapper<TYPE, Delete<TYPE>>;
...
template <typename TYPE>
using NewExecutableHolder = PreallocatedWrapper<TYPE, DeleteExecutable<TYPE>>;
...
template <typename TYPE>
using NewArrayHolder = PreallocatedWrapper<TYPE, DeleteArray<TYPE>>;
...
template <typename TYPE>
using ResetPointerHolder = PreallocatedWrapper<TYPE, detail::ZeroMem<TYPE>::Invoke>;
template <typename TYPE>
using FieldNuller = PreallocatedWrapper<TYPE, detail::ZeroMem<TYPE>::Invoke>;
...
template <typename TYPE>
using LocalAllocHolder = PreallocatedWrapper<TYPE, DoLocalFree<TYPE>>;
